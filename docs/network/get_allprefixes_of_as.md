...
>  1. Объем: У крупных корпораций (Google, Cloudflare) могут быть тысячи префиксов. Добавление такого количества маршрутов может занять время и «замусорить» таблицу маршрутизации.
>  2. IPv6: Поддержка IPv6 в скрипте закомментирована.
>  3. Зависимости: Скрипт упадет, если API RIPE или сервис Cymru будут недоступны.
>  4. Обратный процесс: В скрипте нет функции удаления добавленных маршрутов — их придется удалять вручную или перезагрузкой (если они не -PolicyStore Persistent, но здесь они создаются в текущей сессии).
