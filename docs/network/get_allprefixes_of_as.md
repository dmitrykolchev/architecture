...
> На что стоит обратить внимание:Объем: У крупных корпораций (Google, Cloudflare, VK) могут быть тысячи префиксов. Добавление такого количества маршрутов может занять время и «замусорить» таблицу маршрутизации.IPv6: Поддержка IPv6 в скрипте закомментирована.Зависимости: Скрипт упадет, если API RIPE или сервис Cymru будут недоступны.Обратный процесс: В скрипте нет функции удаления добавленных маршрутов — их придется удалять вручную или перезагрузкой (если они не -PolicyStore Persistent, но здесь они создаются в текущей сессии).
